...
function add_vote_to_post($post_id) {
	$post = get_post($post_id);
	$c = intval($post->likes_count) + 1; # n+1
	update_post(array('id' => $post_id, 'likes_count' => $c)); 	
...
if (! empty($_GET['id'])) {
	...
	echo "<p>Invalid request parameters. Missing required id param.</p>";
}
