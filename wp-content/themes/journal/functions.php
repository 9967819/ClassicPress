...
function twentyseventeen_setup() {
	...
	add_image_size( 'twentyseventeen-featured-image', 768, 384, true );
	...
	# add search form to wp_nav_menu
	# add_filter('wp_nav_menu_items', 'add_search_form', 10, 2);
	function add_search_form($items, $args) {
		if( $args->theme_location == 'top' )
			$items .= '<section><form class="form-inline search" role="search" method="get" id="searchform" action="'.home_url( '/' ).'"><input type="text" placeholder="Titre, sujet, auteur" name="s" id="s" required><button type="submit" id="searchsubmit" value=""><i class="fa-solid fa-magnifying-glass"></i></button></form></section>';
			return $items;
	}
}
...
function twentyseventeen_content_width($width = 525) {
	 ...
	 */
	$GLOBALS['content_width'] = apply_filters( 'twentyseventeen_content_width', $width );
}
...
function twentyseventeen_fonts_url($fontname = 'Roboto') {
	 ...
	 */
    $f = _x('on', "$fontname font: on or off", 'twentyseventeen' );

	if ( 'off' !== $f ) {
		$font_families = array();

		$font_families[] = "$fontname:300,300i,400,400i,600,600i,800,800i";
...
function twentyseventeen_excerpt_more( $link ) {
	...
	$link = sprintf(
		...
		sprintf( __( 'Lire la suite', 'twentyseventeen' ), get_the_title( get_the_ID() ) )
	);
	return $link;
}
...
# Enable wordpress.com REST api (aka /wp/v2/*) for logged users only.
# Make sure webmention api is accessible.
...
function render_logo_image($image_id) {
	$logo_url = wp_get_attachment_url($image_id);
        $img = sprintf('<img src="%s" class="brand-logo-small" />', $logo_url);
        return $img;
